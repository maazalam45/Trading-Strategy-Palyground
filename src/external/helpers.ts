...
export async function getBarsApiRequest(path: string, type: 'hist'): Promise<any> {
  const url = getCandleURL(type, path)
  const r = await fetch("https://candle.4xbrokers.com/api/candles?fsym=EUR&tsym=USD&toTs=1999999999&limit=2000&resolution=1M&tt=r")
