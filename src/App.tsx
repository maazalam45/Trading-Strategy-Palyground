      ...
      />
        ...
        <div>
          ...
          <select value={interval} onChange={e => setIntervalTF(e.target.value)}>
            {['1','5','15','30','60','1H','4H','1D','1W','1M'].map(tf => <option key={tf} value={tf}>{tf}</option>)}
          </select>
        </div>
        <div style={{marginLeft:'auto', opacity:.8}}>Use built‑in toolbar for Ruler, Price/Date Range, Line, Trendline, Fib…</div>
      </div>
