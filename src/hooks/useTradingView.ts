...
export function useTradingView(containerId: string, symbol: string, interval: string, extra?: Record<string, any>) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        ;(widget.onChartReady || widget.onready)?.(() => mounted && setReady(true))
